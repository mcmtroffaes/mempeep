...
local function new(compute, mem)
    ...
    local ptr_size = compute.sizeof({kind = "ptr", type_ref = {kind = "i8"}, optional = false, weak = true})

    local read  -- forward declaration for mutual recursion

    local readers = {}

    readers.array = function(addr, type_ref)
    ...
    readers.circular_list = function(addr, type_ref)
        local sub_type = type_ref.type_ref
        local items    = {}
    ...
    readers.string = function(addr, type_ref)
    ...
    readers.struct = function(addr, type_ref)
    ...
    readers.vector = function(addr, type_ref)
